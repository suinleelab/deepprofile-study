###############################
#Script for encoding GTEX expression using VAE models
###############################
...
data_folder = '../ALL_CANCER_FILES/' + cancer + '/'

#Read GTEX expression
input_df_test = pd.read_table(data_folder + 'HEALTHY_TISSUE_FILES/GTEX_' + cancer + '_DATA_1K_PCs.tsv', index_col = 0)    
print("GTEX expression dataframe ", input_df_test.shape)
        
#Encode expression data with each VAE model
for fold in range(start, end):
    print("VAE model with " + str(dimension) + " nodes and fold " + str(fold))

    #Load VAE models
    json_file = open(data_folder + 'VAE_FILES/VAE_' + cancer + '_encoder_' + str(dimension) + 'L_' + str(fold) + '.json', 'r')   
    loaded_model_json = json_file.read()
    json_file.close()
    encoder = model_from_json(loaded_model_json)

    encoder.load_weights(data_folder + 'VAE_FILES/VAE_' + cancer + '_encoder_' + str(dimension) + 'L_' + str(fold) + '.h5')
    ...
    test_encoded_df.to_csv(data_folder + 'HEALTHY_TISSUE_FILES/' + 'GTEX_' + cancer + '_RNASeq_Expression_VAE_encoded_' + str(dimension) + 'L_' + str(fold) + '.tsv', sep = '\t')
