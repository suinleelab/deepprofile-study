###############################
#Script for creating GTEX DeepProfile embeddings
###############################
...
for dim in dims:
    for i in range(run):
        data_df = pd.read_table(input_folder + 'HEALTHY_TISSUE_FILES/GTEX_' + cancer_type + '_RNASeq_Expression_VAE_encoded_' + str(dim) + 'L_' + str(i) + '.tsv', index_col = 0)      
        print("GTEX VAE embedding ", data_df.shape)
...
print("DeepProfile ensemble embedding ", ensemble_embeddings.shape)
ensemble_embeddings_df = pd.DataFrame(ensemble_embeddings, index = data_df.index, columns = np.arange(L))
ensemble_embeddings_df.to_csv(output_folder + cancer_type + '_DeepProfile_GTEX_Healthy_Tissue_Embedding_' + str(L) + 'L.tsv', sep = '\t')