...
output_folder = '../ALL_CANCER_FILES/' + cancer_type + '/DAE_FILES/'
...
#Define noisy inputs
noise = np.random.normal(loc=0, scale = 1, size=input_df_training.shape)
input_df_noisy = input_df_training.values + noise
...
#DAE model
dae = Model(x, x_decoded_mean)

adam = optimizers.Adam(lr=learning_rate)
dae.compile(optimizer=adam, loss = reconstruction_loss)
dae.summary()


#Train from only training data
history  = dae.fit(np.array(input_df_noisy), np.array(input_df_training),
               shuffle=True,
               epochs=epochs,
...
with open(output_folder + "DAE_" + cancer_type + "_encoder_" + str(latent_dim) + "L_"+ str(fold) + ".json", "w") as json_file:
    json_file.write(model_json)

encoder.save_weights(output_folder + "DAE_" + cancer_type + "_encoder_" + str(latent_dim) + "L_"+ str(fold) + ".h5")
print("Saved model to disk")


model_json = decoder.to_json()
with open(output_folder + "DAE_" + cancer_type + "_decoder_" + str(latent_dim) + "L_"+ str(fold) + ".json", "w") as json_file:
    json_file.write(model_json)

decoder.save_weights(output_folder + "DAE_" + cancer_type + "_decoder_" + str(latent_dim) + "L_"+ str(fold) + ".h5")
print("Saved model to disk")
