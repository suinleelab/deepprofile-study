...
#Load model
json_file = open(input_folder 'AE_FILES/AE_' + cancer + '_encoder_' + str(dimension) + 'L_' + str(vae_run) + '.json', 'r')
loaded_model_json = json_file.read()
json_file.close()

encoder = model_from_json(loaded_model_json)
encoder.load_weights(input_folder + 'AE_FILES/AE_' + cancer  + '_encoder_' + str(dimension) + 'L_' + str(vae_run) + '.h5')
...
ig_df.to_csv(output_folder + cancer + '_DATA_AE_Weights_TRAINING_' + str(dimension) + 'L_fold' + str(vae_run) + '.tsv', sep='\t', quoting = csv.QUOTE_NONE)
print(ig_df.shape)
