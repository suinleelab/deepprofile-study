...
#Encode test data with all 10 AE models
for fold in range(start, end):
    print("AE model with " + str(dimension) + " nodes and fold " + str(fold))
    
    #Load AE models
    json_file = open(input_folder + 'AE_FILES/AE_' + cancer + '_encoder_' + str(dimension) + 'L_' + str(fold) + '.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    encoder = model_from_json(loaded_model_json)
    
    encoder.load_weights(input_folder + 'AE_FILES/AE_' + cancer + '_encoder_' + str(dimension) + 'L_' + str(fold) + '.h5')
    print("Loaded model from disk")

    # Encode test data using the AE model
    ...
    test_encoded_df.to_csv(output_folder + 'TCGA_' + tcga_name + '_RNASeq_Expression_AE_encoded_' + str(dimension) + 'L_' + str(fold) + '.tsv', sep = '\t')
