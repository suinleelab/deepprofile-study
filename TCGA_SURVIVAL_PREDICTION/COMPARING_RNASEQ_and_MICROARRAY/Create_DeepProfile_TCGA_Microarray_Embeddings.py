...
for dim in dims:
    for i in range(run):
        data_df = pd.read_table(input_folder+ 'TCGA_FILES/TCGA_' + tcga_type + '_MICROARRAY_Expression_VAE_encoded_' + str(dim) + 'L_' + str(i) + '.tsv', index_col = 0)   
...
ensemble_embeddings_df.to_csv(output_folder + tcga_type + '_DeepProfile_TCGA_MICROARRAY_Embedding_' + str(L) + 'L.tsv', sep = '\t')