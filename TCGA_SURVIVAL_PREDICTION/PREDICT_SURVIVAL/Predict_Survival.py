...
def predict_survival(cancer_type, tcga_type, method, run_index, seed):
    ...
    print("Expression data ", X_df.shape)
    print("Expression data ", X_df.index)
...
tcga_type = sys.argv[2] #TCGA type
method = sys.argv[3] #name of the method
run_index = int(sys.argv[4]) #run index
if len(sys.argv) > 5:
    VAE_dim = sys.argv[5]
...
#print( Y_df[Y_df['fustat']  == 1]['futime'])
...
for sampling_index in range(50):
    result = predict_survival(cancer_type, tcga_type, method, run_index, sampling_index)
...
if method == 'VAE':
    np.savetxt(output_folder + cancer_type + '/TCGA_Survival_5year_LR_Balanced_Subsample_20FOLD_50Runs_' + tcga_type + '_' + method + '_' + VAE_dim + 'L_' + str(run_index) + '_ACCs.txt', np.asarray(all_accuracies), delimiter='\n')
    np.savetxt(output_folder + cancer_type + '/TCGA_Survival_5year_LR_Balanced_Subsample_20FOLD_50Runs_' + tcga_type + '_' + method + '_' + VAE_dim + 'L_' + str(run_index) + '_AUCs.txt', np.asarray(all_aucs), delimiter='\n')

else:
    np.savetxt(output_folder + cancer_type + '/TCGA_Survival_5year_LR_Balanced_Subsample_20FOLD_50Runs_' + tcga_type + '_' + method + '_' + str(run_index) + '_ACCs.txt', np.asarray(all_accuracies), delimiter='\n')
    np.savetxt(output_folder + cancer_type + '/TCGA_Survival_5year_LR_Balanced_Subsample_20FOLD_50Runs_' + tcga_type + '_' + method + '_' + str(run_index) + '_AUCs.txt', np.asarray(all_aucs), delimiter='\n')
